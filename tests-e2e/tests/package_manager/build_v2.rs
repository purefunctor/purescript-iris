...
fn builds_the_selected_workspace_package() {
    ...
    let output = workspace.v2_command_in("packages/application/src", &["build", "--quiet"]);
...
fn builds_the_whole_workspace_from_a_root_package_subdirectory() {
    ...
    let output = workspace.v2_command_in("src", &["build", "--quiet"]);
...
fn excludes_gitignored_packages_from_selection() {
    ...
    let output = workspace.v2_command(&["build", "--quiet", "--package", "ignored"]);
...
fn excludes_nested_workspaces_from_selection() {
    ...
    let output = workspace.v2_command(&["build", "--quiet", "--package", "child"]);
...
fn suppresses_diagnostics_without_changing_failure_status() {
    ...
    let output = workspace.v2_command(&["build", "--quiet", "--no-diagnostics"]);

    snapshot_output("v2_suppressed_error_diagnostics", &output);
}
